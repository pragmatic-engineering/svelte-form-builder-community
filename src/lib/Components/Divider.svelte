<script lang="ts">
	...
	let height: string;
	$: {
		if (field.dataAttributes?.find((x) => x.name == 'customSize')?.value) {
			height = `${field.dataAttributes?.find((x) => x.name == 'customSize')?.value}px`;
		} else {
			height = `${field.dataAttributes?.find((x) => x.name == 'size')?.value}px`;
		}
	}
</script>
